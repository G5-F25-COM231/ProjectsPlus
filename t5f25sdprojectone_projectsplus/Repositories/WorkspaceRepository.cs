...
namespace t5f25sdprojectone_projectsplus.Repositories
{
    public class WorkspaceRepository : IWorkspaceRepository
    {
        ...
        // Interface-compatible CreateAsync (previously InsertAsync)
        ...
        public async Task<WorkspaceEntity> UpdateAsync(WorkspaceEntity workspace, CancellationToken ct = default)
        {
            ...
            var updated = await _db.Workspaces.FirstOrDefaultAsync(w => w.Id == workspace.Id, ct).ConfigureAwait(false);
            return updated!;
        }

        // Interface-compatible GetByIdAsync
        ...
        // Interface-compatible FindBySlugAsync (global slug lookup)
        public async Task<WorkspaceEntity?> FindBySlugAsync(string slug, CancellationToken ct = default)
        {
            ...
            return await _db.Workspaces
                .AsNoTracking()
                .FirstOrDefaultAsync(w => w.Slug == s && !w.IsDeleted, ct)
                .ConfigureAwait(false);
        }

        // Project-scoped lookup retained (useful internally)
        public async Task<WorkspaceEntity?> FindByProjectAndSlugAsync(long projectId, string slug, CancellationToken ct = default)
        {
            ...
            return await _db.Workspaces
                .AsNoTracking()
                .FirstOrDefaultAsync(w => w.ProjectId == projectId && w.Slug == s && !w.IsDeleted, ct)
                .ConfigureAwait(false);
        }

        // Interface-compatible ListByOwnerAsync (owner-scoped listing)
        public async Task<IReadOnlyList<WorkspaceEntity>> ListByOwnerAsync(long ownerUserId, CancellationToken ct = default)
        {
            var list = await _db.Workspaces
                .AsNoTracking()
                .Where(w => w.OwnerUserId == ownerUserId && !w.IsDeleted)
        ...
        // Project-scoped listing (internal/backwards-compatible)
        ...
        // State transition helper (keeps original behavior)
        public async Task<WorkspaceEntity> TransitionStateAsync(long workspaceId, WorkspaceState targetState, string metadataJson, int expectedVersion, CancellationToken ct = default)
        {
            ...
            var updated = await _db.Workspaces.FirstOrDefaultAsync(w => w.Id == workspaceId, ct).ConfigureAwait(false);
            return updated!;
        }

        // Interface-compatible DeleteAsync with expectedVersion
        ...
        // Helper used to map DB constraint violations to domain exception
